...
def home():
    result = {}
    with requests.get(url=wowAPI.guild_info_uri + wowAPI.key) as request:
        content = json.loads(request.text)
        result['guild_info'] = content
    with requests.get(url=wowAPI.guild_members_uri + wowAPI.key) as request:
        content = json.loads(request.text)
    ...
    with requests.get(url="https://www.wowprogress.com/guild/eu/arathor/The+Forgotten+Few/json_rank") as request:
        content = json.loads(request.text)
...
def member():
    ...
    with requests.get(url="{}character/{}/{}?fields=items&locale=en_GB&apikey={}".format(wowAPI.base_uri, realm, name, wowAPI.key)) as request:
        content = json.loads(request.text)
...
@app.route("/news")
def news():
    with requests.get(url=wowAPI.guild_news_uri + wowAPI.key) as request:
        result = {}
